...
export class MoviesDataService {
    ...
    private static transformResponseToMoviesObject = (data: DataRest): Omit<LoadedMovie, 'key'>[] => (
      data.results.map(value => ({
        ...
        img: (value.backdrop_path && this.getImageFromPath(value.backdrop_path)) ||
             (value.poster_path && this.getImageFromPath(value.poster_path)),
        name: value.name || value.original_name || value.title || value.original_title || "Noname",
        releaseDate: (value?.release_date && dayjs(value?.release_date)) ?? 'Unknown'
      }))
    )

    private static getImageFromPath = (path: string) => `${this.middleResImageUrl}${path}`
